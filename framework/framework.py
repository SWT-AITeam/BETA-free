...
class Framework(object):
    ...
    def train(self, model_pattern,pre_train_model_name):
        ...
        if self.config.multi_gpu:
            ...
            model = ori_model
        if pre_train_model_name is not None:
            print(f"load pre_train_model_name from {pre_train_model_name}")
            path = os.path.join(self.config.checkpoint_dir, pre_train_model_name)
        ...
        for epoch in range(self.config.max_epoch):
            ...
            while data is not None:
                ...
                pred_triple_matrix,kld = model(data)
                l = (kld*data['loss_mask'].view(kld.shape[0],-1)).sum()*self.config.beta

                #triple_loss = cal_loss(data['triple_matrix'], pred_triple_matrix, data['loss_mask'])+(kld*data['loss_mask']).sum()*self.config.beta
                triple_loss = cal_loss(data['triple_matrix'], pred_triple_matrix, data['loss_mask'])+l
    ...
    def test(self, test_data_loader, model, current_f1, output=True):
        ...
        def to_ht(triple_list):
            ret = []
            for triple in triple_list:
                ret.append((triple[0], triple[2]))
            return ret

        def to_r(triple_list):
            ret = []
            for triple in triple_list:
                ret.append((triple[1],))
            return ret
        ...
        ht_correct_num, ht_predict_num, ht_gold_num = 0, 0, 0
        r_correct_num, r_predict_num, r_gold_num = 0, 0, 0
        results = []
        test_num = 0
        ...
        while data is not None:
            ...
            with torch.no_grad():
                ...
                ht_pred_list = list(ht_set)
                pred_ht = set(ht_pred_list)
                ht_gold_list = set(to_ht(data['triples'][0]))

                ht_correct_num += len(pred_ht & ht_gold_list)
                ht_predict_num += len(pred_ht)
                ht_gold_num += len(ht_gold_list)

                r_pred_list = list(r_set)
                pred_r = set(r_pred_list)
                r_gold_list = set(to_r(data['triples'][0]))

                r_correct_num += len(pred_r & r_gold_list)
                r_predict_num += len(pred_r)
                r_gold_num += len(r_gold_list)
        ...
        ht_precision = ht_correct_num / (ht_predict_num + 1e-10)
        ht_recall = ht_correct_num / (ht_gold_num + 1e-10)
        ht_f1_score = 2 * ht_precision * ht_recall / (ht_precision + ht_recall + 1e-10)

        r_precision = r_correct_num / (r_predict_num + 1e-10)
        r_recall = r_correct_num / (r_gold_num + 1e-10)
        r_f1_score = 2 * r_precision * r_recall / (r_precision + r_recall + 1e-10)
        self.logging("ht_f1_score: {:4.4f}, ht_precision: {:4.4f}, ht_recall: {:4.4f}".format(ht_f1_score, ht_precision,
                                                                                       ht_recall))
        self.logging("r_f1_score: {:4.4f}, r_precision: {:4.4f}, r_recall: {:4.4f}".format(r_f1_score, r_precision, r_recall))
