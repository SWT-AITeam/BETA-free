...
valid_ids = []
for item in tokens:
    if item.startswith("##") or item=="[unused1]" or item=="[SEP]" or item=="[CLS]":
        valid_ids.append(0)
    else:
        valid_ids.append(1)
...
def re_collate_fn(batch):
    ...
    batch_triple_matrix = torch.LongTensor(cur_batch_len, 216, max_text_len, max_text_len).zero_()
...
def get_loader(config, prefix, is_test=False, num_workers=0, collate_fn=re_collate_fn):
    ...
    return data_loader
"""
def re_collate_fn_wrapper(rel_num):
    def re_collate_fn(batch):
        batch = list(filter(lambda x: x is not None, batch))
        batch.sort(key=lambda x: x[3], reverse=True)
        ...
        return {'token_ids': batch_token_ids,
                ...
                'triples': triples,
                'tokens': tokens}
    return re_collate_fn





def get_loader(config, prefix, is_test=False, num_workers=0, collate_fn=re_collate_fn_wrapper):
    dataset = REDataset(config, prefix, is_test, tokenizer)
    if not is_test:
        data_loader = DataLoader(dataset=dataset,
                                 batch_size=config.batch_size,
                                 ...
                                 pin_memory=True,
                                 num_workers=num_workers,
                                 collate_fn=collate_fn(config.rel_num))
    else:
        data_loader = DataLoader(dataset=dataset,
                                 batch_size=1,
                                 shuffle=False,
